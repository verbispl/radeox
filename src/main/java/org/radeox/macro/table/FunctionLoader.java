 ...
 */
public class FunctionLoader extends PluginLoader<Function>
{
    private static Log log = LogFactory.getLog(FunctionLoader.class);

    protected static FunctionLoader instance;

    public static synchronized PluginLoader<Function> getInstance()
    {
        if(null == instance)
        {
    ...
    @Override
    public Class<Function> getLoadClass()
    {
     ...
     */
    @Override
    public <R extends Function> void add(final Repository<Function> repository, final R plugin)
    {
        if(plugin instanceof Function)
        {
            repository.put(plugin.getName().toLowerCase(), plugin);
        }
        else
        {
